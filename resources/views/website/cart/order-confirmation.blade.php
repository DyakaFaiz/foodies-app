...
<section class="shop-checkout container">
    ...
    <div class="order-info">
        ...
        <div class="order-info__item">
        <label>Date</label>
        <span>{{ $pesanan->created_at }}</span>
        </div>
        <div class="order-info__item">
        <label>Total</label>
        <span>{{ number_format($pesanan->total, 0, ',', '.') }}</span>
        </div>
        <div class="order-info__item">
        <label>Paymetn Method</label>
        <span>{{ $pesanan->metode_pembayaran == 0 ? "Cash On Delivery" : "transfer" }}</span>
        </div>
    </div>
        ...
        <table class="checkout-cart-items">
            ...
            <tbody>
                @foreach ( $detailPesanan as $row)
                    <tr>
                        <td>
                            {{ $row->namaProduk }} x {{ $row->jumlah > 1 ? $row->jumlah : '' }}
                        </td>
                        <td>
